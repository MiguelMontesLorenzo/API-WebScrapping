...
def extract():
    ...
    lst = { string.split(sep)[0].strip().lower(): float(string.split(sep)[1]) for string in return_all_li(find_ul_tag(divs)) }
    ...
    while not finish:
        ...
        if selection in dict_of_nba_teams:
            ...
            selected_team_name = dict_of_nba_teams[selected_team_id]['name']
            finish = True
    ...
    URL = f'https://api.sportsdata.io/v3/nba/odds/json/TeamTrends/{team_abbreviation}'
    API_response = requests.get(url = URL, headers = headers)
    trends_info = API_response.json()

    opponent_abbreviation = trends_info['UpcomingGame']['AwayTeam'] if not trends_info['UpcomingGame']['AwayTeam'] == team_abbreviation else trends_info['UpcomingGame']['HomeTeam']

    for opp_id, team_info in dict_of_nba_teams.items():
        if team_info['abbreviation'] == opponent_abbreviation:
            opponent_id = opp_id
            opponent_name = team_info['name']


    selected_team_chances = float(lst[selected_team_name.lower()]) if selected_team_name.lower() in lst else 4
    opponent_chances = float(lst[opponent_name.lower()]) if opponent_name.lower() in lst else 4
    win_probability = selected_team_chances / (opponent_chances + selected_team_chances) 


    print(f'next_opponent: {opponent_name}     - winning_probability: {win_probability}')
    ...
    return dict_of_nba_teams, selected_team_id, opponent_name, win_probability
...
def load(df1, df2, df3, dict_of_nba_teams, selected_team_id, next_opponent_name, win_probability):



    # CREATE FIGURE 1: POINTS FOR AND AGAINST
    print(df1)
    df1a = df1[['opponent name', 'total points']]
    fig1a = px.bar( data_frame=df1a,
                    x='opponent name',
                    y=df1a.columns,
    ...
    df1b = df1[['opponent name', 'opponent points']]
    fig1b = px.bar( data_frame=df1b,
                    x='opponent name',
                    y=df1b.columns,
                    title='Orders by pizza size',
    ...
    # Obtain next opponent
    next_opponent = next_opponent_name
...
if __name__ == '__main__':
    ...
    nba_teams_data, selected_team_id, opponent, win_probability = extract()
    df1, df2, df3 = transform(nba_teams_data, selected_team_id)
    load(df1, df2, df3, nba_teams_data, selected_team_id, opponent, win_probability)